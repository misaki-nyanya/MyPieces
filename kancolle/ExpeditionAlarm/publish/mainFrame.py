...
def startout(seconds,name,child_conn):
    freeze_support()
...
class alarmCenter:
    ...
    def terminateAll(self):
        for each in self.alarmList.keys():
            self.alarmList[each]=None

    def listen(self,alarms):
        print 'listener:pid '+str(os.getpid())+' is created'
        while True:
            now=time.time()
            print 'listening at '+str(now)
            print '--------------listenerList Below-------------'
            for each in alarms.keys():
                print each+' '+str(alarms[each])
            print '--------------listenerList above-------------'
            for each in alarms.keys():
                if alarms[each]!=None:
                    if alarms[each]<now:
                        winsound.PlaySound('SystemQuestion', winsound.SND_ALIAS)
